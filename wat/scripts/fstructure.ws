...
function replace_brackets(str){
	...
	var tagnf=WAT.document.getElementsByTagName('noframes');nf=0;
...
else {stuff+='<li>Longdesc <strong style=\"color:#ff0000\">'+localize('frame4')+'</strong></li>'}

if (tagf[ippo].attributes.title.specified==true){
stuff+='<li>Title="'+tagf[ippo].title+'"</li>'
}
else {stuff+='<li>Title <strong style=\"color:#ff0000\">'+localize('frame4')+'</strong></li>'}        
...
return;
	...
	try{ var lw=window.open('about:blank','');
		lw.document.open();}
		catch(e) {
		if (lw){
