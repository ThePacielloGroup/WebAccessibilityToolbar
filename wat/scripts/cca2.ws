...
//first check page for frames
	var ifrm=WAT.document.getElementsByTagName('iframe');   // change 'frame' to 'iframe'
	//alert("IFrame length is " + ifrm.length);
	var f_loop = 1;
	if (ifrm.length>0)
	{
		alert(localize('errortxt6'));   //This function encountered iframes and may not work
		if (f_length > 1)		// there are multiple iframes counted in the f_length property, but don't know where this happens
			f_loop = 1;			// we only want to do the one main frame, else the code below chokes
		else
			f_loop = f_length;
	}

for(fr = 0; fr <f_loop; fr++)
//for(fr = 0; fr < 1 ; fr++)
{ //2
