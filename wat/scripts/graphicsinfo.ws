// applied changes May 2014 to prevent multiple windows when encounters iFrames...
 
function main()
{
    var doc = false;  // add to make the window opening work
	
    for(fr = 0; fr <f_length; fr++)
    {
		//alert("frame count is " + fr); //yes, frame counter here...
        if(!'push' in [])
		...
		try
			...
			{
			//alert("Page had problem with " + d[i].tagName);
			alert(localize('errortxt2') + ", -> " + d[i].tagName);
			}
		}
		...
		if (typeof document.body.style.maxHeight=="undefined") 
			...
			try
				...
				//alert ("doc is " + doc);
				doc.document.write(out1+out2+out3);
				if (fr == 0)
					{
